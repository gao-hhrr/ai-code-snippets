...
import FolderRow from '@/components/snippet/FolderRow.vue'
...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape' && creatingFolder.value) cancelCreateFolder()
}

// 新建气泡用 document 级 mousedown：按下点在气泡外则关闭。
// 不能用 click：拖选文字在气泡内按下、气泡外松开时，click 落在共同祖先上（判为"外部"）会误关编辑态
function onDocMouseDownNewFolder(e: MouseEvent) {
  if (!creatingFolder.value) return
  if (!(e.target as HTMLElement).closest('.new-folder-bubble')) {
    cancelCreateFolder()
  }
}

onMounted(() => {
  document.addEventListener('mousedown', onDocMouseDownNewFolder, true)
  document.addEventListener('keydown', onKeydown)
})
onBeforeUnmount(() => {
  document.removeEventListener('mousedown', onDocMouseDownNewFolder, true)
