...
function onDocClickSort(e: MouseEvent) {
  if (!(e.target as HTMLElement).closest('.sort-menu')) {
    showSortMenu.value = false
  }
}

function onKeydownSort(e: KeyboardEvent) {
  if (e.key === 'Escape') showSortMenu.value = false
}

onMounted(() => {
  document.addEventListener('click', onDocClickSort, true)
  document.addEventListener('keydown', onKeydownSort)
})
onBeforeUnmount(() => {
  document.removeEventListener('click', onDocClickSort, true)
  document.removeEventListener('keydown', onKeydownSort)
})
</script>
