...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'Escape') {
    menuOpen.value = false
    if (editing.value) cancelRename()
  }
}

// 不用全屏遮罩，改用 document 级 click 监听点击外部关闭。
// 仅在菜单打开时拦截，避免挡住 more 图标按钮本身打开菜单的点击。
function onDocClickMenu(e: MouseEvent) {
...
function onDocMouseDownEdit(e: MouseEvent) {
  if (!editing.value) return
  if (!(e.target as HTMLElement).closest('.edit-bubble')) {
    cancelRename()
  }
}
...
onMounted(() => {
  ...
  document.addEventListener('mousedown', onDocMouseDownEdit, true)
  document.addEventListener('keydown', onKeydown)
})
onBeforeUnmount(() => {
  document.removeEventListener('click', onDocClickMenu, true)
  document.removeEventListener('mousedown', onDocMouseDownEdit, true)
