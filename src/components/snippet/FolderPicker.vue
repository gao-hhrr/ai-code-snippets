...
// 不用全屏遮罩（会挡住下方按钮的光标/hover 导致"抽搐"）。
// 改为 document 级 click 监听：点击气泡外立即关闭，capture 阶段阻止事件继续传播，
// 避免关闭瞬间的这次点击穿透到下面的按钮（如卡片跳转、再次点开星标）。
function onDocClick(e: MouseEvent) {
  if (!pickerEl.value?.contains(e.target as Node)) {
