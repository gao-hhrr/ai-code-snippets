// ════════════════════════════════════════════════════════
// api/assistantPrompt.ts —— AI 助手 prompt 组装：候选片段 + 对话历史 + 当前消息 → 发给模型的指令文本
// ════════════════════════════════════════════════════════
...
export function buildAssistantPrompt(p: BuildAssistantPromptParams): string {
  ...
  const prompt = [
    ...
    '',
    '请判断用户本轮想做什么，然后调用对应的工具（tools 里的 search / summarize / modify / operate / ask / chat 之一）：',
    '把结果放进该工具的 parameters：ids 是片段编号 1..N、note 是筛选依据/操作提醒、text 是分析、requirement 是修改需求、op/value/target/field/language 是库操作参数、question 是澄清提问、reply 是寒暄回复。',
    '只调用一个工具，选择规则：明确找片段 → search；总结讲解对比 → summarize；修改代码 → modify；删除/收藏/新建/清空/收藏夹管理/改描述语言 → operate；意图模糊（"你好""随便看看"）→ ask；与找片段无关 → chat。',
    '规则：',
    ...
    '- 修改/库操作必须先明确目标与做法：用户只说"帮我改下代码/优化一下/改好看点"这类空泛需求、没说清改成什么样（或没说改哪个片段）→ 输出 ask 追问（如"你想把哪段代码改成什么样？"），不要臆测需求直接 modify；说清了"改哪个 + 怎么改"才输出 modify。',
    ...
    '- ops 里除 create（新建代码，只允许放第一项，表示需求含新建代码）外，其余步骤只限可逆操作：收藏/取消收藏/导出/新建收藏夹/改名/移动/改描述语言。需求含删除/清空时不要用 ops——先做可逆部分并说明，让用户单独确认不可逆部分。',
