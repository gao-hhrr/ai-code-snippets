...
export interface BuildAssistantPromptParams {
  ...
  message: string
  codeLimit: number
  // 整场会话此前找到过的片段编号并集（已映射到本轮候选）：追问只能在这批编号里筛
...
export function buildSystemPrompt(): string {
  return [
    ...
    '',
    '## 追问与换话题',
    '- 追问（「更简单点的」「有注释的呢」）：结合上文，在此前找到过的片段编号里筛（收窄或回退）；明显换话题/出现新主题词才可越界。',
...
export function buildUserPrompt(p: BuildAssistantPromptParams): string {
  ...
  if (p.lastSearchNums.length > 0) {
    sections.push(
      '',
      '## 对话中此前找到的片段编号',
      `本次对话中此前找到的片段编号是：${p.lastSearchNums.join('、')}。追问（「更简单点的」「有注释的呢」「防抖的呢」）在这些编号里筛：想继续收窄就筛，想反过来调整（「更简单/更复杂/更好看」）也可以在这些编号里回退选择。不得返回这些编号以外的片段；只有明显换话题/出现新主题词时才可找其他编号。`
    )
  }
